...
Track::~Track(void)
{
	...
	glDeleteLists(track_list2, 1);
	glDeleteLists(track_list3, 1);
	glDeleteLists(track_list4, 1);
	glDeleteLists(track_list5, 1);
...
Track::Initialize(void)
{
	...
	glBegin(GL_QUAD_STRIP);
		...
		glVertex3f(p[0], p[1] - 0.45, p[2]);
		glVertex3f(p[0], p[1] - 0.45, p[2] - 0.4);
		glVertex3f(p[0], p[1] - 0.2, p[2]);
		glVertex3f(p[0], p[1] - 0.2, p[2] - 0.4);
	    }
	glEnd();
    glEndList();

	track_list2 = glGenLists(1);
    glNewList(track_list2, GL_COMPILE);
	glColor3f(0.6f, 0.15f, 0.15f);
	glBegin(GL_QUAD_STRIP);
	    for ( i = 0 ; i <= n_refined ; i++ )
	    {
	...
	track_list3 = glGenLists(1);
    glNewList(track_list3, GL_COMPILE);
	glColor3f(1.0f, 1.0f, 1.0f);
	glBegin(GL_QUAD_STRIP);
	    for ( i = 0 ; i <= n_refined ; i++ )
	    {
		refined.Evaluate_Point((float)i, p);
		glVertex3f(p[0], p[1] - 0.2, p[2] - 0.10);
		glVertex3f(p[0], p[1] - 0.2, p[2] - 0.30);
		glVertex3f(p[0], p[1] + 0.2, p[2] - 0.10);
		glVertex3f(p[0], p[1] + 0.2, p[2] - 0.30);
	    }
	glEnd();
	...
	track_list4 = glGenLists(1);
    glNewList(track_list4, GL_COMPILE);
	glColor3f(0.45f, 0.2f, 0.2f);
	glBegin(GL_QUAD_STRIP);
	    for ( i = 0 ; i <= n_refined ; i++ )
	    {
		refined.Evaluate_Point((float)i, p);
		glVertex3f(p[0], p[1] - 0.45, p[2] - 0.4);
		glVertex3f(p[0], p[1] - 0.45, 0.0);
		glVertex3f(p[0], p[1] - 0.2, p[2] - 0.4);
		glVertex3f(p[0], p[1] - 0.2, 0.0);
	    }
	glEnd();
    glEndList();

	track_list5 = glGenLists(1);
    glNewList(track_list5, GL_COMPILE);
	glColor3f(0.45f, 0.2f, 0.2f);
	glBegin(GL_QUAD_STRIP);
	    for ( i = 0 ; i <= n_refined ; i++ )
	    {
...
Track::Draw(void)
{
    ...
    // Draw the track
    glCallList(track_list2);

	glPushMatrix();

    // Draw the track
    glCallList(track_list3);

	glPushMatrix();

    // Draw the track
    glCallList(track_list4);

    glPushMatrix();

	// Draw the track
    glCallList(track_list5);
